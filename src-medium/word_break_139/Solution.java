...
public class Solution {
     ...
     * @runtime O(n*m*l), where n is s.length(), m is max_word_length in wordDict, l
     *          is wordDict.size().
     */
    ...
    public boolean wordBreak1(String s, List<String> wordDict) {
        ...
        while(r <= s.length()) {
            if(!visited.contains(r) && wordDict.contains(s.substring(l, r))) { // if finds a match, push left/right indices to the stack and update
                ...
                if(!stack.empty() && (r == s.length() || r - l >= max_word_length)) { // if reaches the end or larger than max word length, reverse
                                                                                      // the last matched indices
                    r = stack.pop();
                    l = stack.pop();
                    visited.add(r);
                }
...
//		System.out.println(stack);
        ...
        if(!stack.isEmpty() && stack.pop() == s.length()) // if the right index of the last matched substring is at the rightmost index,
                                                          // then s can be segmented
            return true;
