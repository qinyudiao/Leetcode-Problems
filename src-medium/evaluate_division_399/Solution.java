...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        // [3.75, 0.4, 5.0, 0.2]
        // [0.5, 2.0, -1.0, -1.0]
        String[][][] testCasesEquations = { { { "a", "b" }, { "b", "c" } },
                { { "a", "b" }, { "b", "c" }, { "bc", "cd" } }, { { "a", "b" } } };
        ...
        for(int i = 0; i < testCasesValues.length; i++) {
            List<List<String>> equations = Arrays.stream(testCasesEquations[i]).map(Arrays::asList)
                    .collect(Collectors.toList());
            List<List<String>> queries = Arrays.stream(testCasesQueries[i]).map(Arrays::asList)
                    .collect(Collectors.toList());
...
class Node {
    ...
    public String toString() {
        return new StringBuilder().append("id: ").append(id).append(", neighbors: ").append(neighbors.values())
                .toString();
    }
}
